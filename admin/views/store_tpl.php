...
<div class="d-flex flex-column flex-sm-row justify-content-between mb-4 ml-1">
    <div class="mb-3 mb-sm-0">
        ...
        <a href="./store.php?action=tpl&tag=paid_top" class="badge badge-light text-primary p-2 small">🏆 销量排行</a>
    </div>
                                ...
                                <div>
                                    ...
                                    <?php if ($v['price'] > 0): ?>
                                        <?php if ($v['purchased'] === true): ?>
                                            ...
                                            <a href="#" class="btn btn-success installBtn" data-url="<?= urlencode($v['download_url']) ?>" data-cdn-url="<?= urlencode($v['cdn_download_url']) ?>" data-type="tpl">安装</a>
                                        <?php elseif ($v['svip'] && Register::getRegType() === 2): ?>
                                            <a href="#" class="btn btn-warning installBtn" data-url="<?= urlencode($v['download_url']) ?>" data-cdn-url="<?= urlencode($v['cdn_download_url']) ?>" data-type="tpl">安装</a>
                                        <?php else: ?>
                    ...
                    if (response.code === 200 && response.data.apps.length > 0) {
                        ...
                        response.data.apps.forEach(function(tpl) {
                            ...
                            html += `
                                <div class="col-md-6 col-lg-3">
                                    <div class="card mb-4 shadow-sm hover-shadow-lg">
                                        <a href="#appModal" class="p-1" data-toggle="modal" data-target="#appModal" data-name="${tpl.name}" data-url="${tpl.app_url}" data-buy-url="${tpl.buy_url}">
                                            <img class="bd-placeholder-img card-img-top" alt="cover" width="100%" height="225" src="${icon}">
                                        </a>
                                        <div class="card-body">
                                            <p class="card-text font-weight-bold">
                                                ${tpl.top === 1 ? '<span class="badge badge-pink p-1">今日推荐</span>' : ''}
                                                <a href="#appModal" data-toggle="modal" data-target="#appModal" data-name="${tpl.name}" data-url="${tpl.app_url}" data-buy-url="${tpl.buy_url}">${tpl.name.substring(0, 15)}</a>
                                                ${tpl.svip ? '<a href="https://www.emlog.net/register" class="badge badge-warning p-1" target="_blank">铁杆免费</a>' : ''}
                                            </p>
                                            <p class="card-text text-muted">
                                                售价：
                                                ${tpl.price > 0 ? 
                                                    (tpl.promo_price > 0 ? 
                                                        `<span style="text-decoration:line-through">${tpl.price}<small>元</small></span> <span class="text-danger">${tpl.promo_price}<small>元</small></span>` : 
                                                        `<span class="text-danger">${tpl.price}<small>元</small></span>`
                                                    ) : 
                                                    '<span class="text-success">免费</span>'
                                                }
                                                <br>
                                                <small>
                                                    开发者：<a href="./store.php?action=tpl&author_id=${tpl.author_id}">${tpl.author}</a><br>
                                                    版本号：${tpl.ver}<br>
                                                    安装次数：${tpl.downloads}<br>
                                                    更新时间：${tpl.update_time}<br>
                                                </small>
                                            </p>
                                            <div class="card-text d-flex justify-content-between">
                                                <div class="installMsg"></div>
