        ...
        <div class="card mb-4 shadow-sm">
            ...
            <div class="card-body">
                <p class="card-text text-muted small">
					<?= $media_name ?><br>
<!--vot-->      <?=lang('upload_time')?>: <?= $value['addtime'] ?><br>
<!--vot-->      <?=lang('file_size')?>: <?= $value['attsize'] ?>,
                </p>
                <p class="card-text d-flex justify-content-between">
					<?php if (isImage($value['mimetype'])): ?>
<!--vot-->              <a href="javascript:insert_media_img('<?= $media_url ?>', '<?= $media_icon ?>')" title="<?=lang('insert_into')?>"><i class="icofont-plus"></i></a>
<!--vot-->              <a href="javascript:insert_conver('<?= $media_path ?>')" title="<?=lang('set_cover')?>"><i class="icofont-image"></i></a>
