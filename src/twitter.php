...
function getindextw()
{
	...
	while ($rows = $DB->fetch_array($query))
	{
		...
		$delbt = ROLE == 'admin' ? "<a href=\"javascript:void(0);\" onclick=\"isdel($id,'twitter')\">{$lang['remove']}</a>" : '';
	...
	{
		$twitter.= "<li><a href=\"javascript:void(0);\" onclick=\"sendinfo('twitter.php?p=$UpPage','twitter')\">&laquo; {$lang['twitter_previous']}</a><small>$page/$pagenums</small><a href=\"javascript:void(0);\" onclick=\"sendinfo('twitter.php?p=$NextPage','twitter')\">{$lang['twitter_next']} &raquo;</a></li>";
	} elseif ($page == 1 && $pagenums > 1) {
		$twitter.= "<li><a href=\"javascript:void(0);\" onclick=\"sendinfo('twitter.php?p=2','twitter')\">{$lang['twitter_next']} &raquo;</a></li>";
	} elseif ($page == $pagenums && $pagenums > 1) {
		$twitter.= "<li><a href=\"javascript:void(0);\" onclick=\"sendinfo('twitter.php?p=$UpPage','twitter')\">&laquo; {$lang['twitter_previous']}</a></li>";
	}
	return $twitter;
}
