...
$lang['db_prefix']	= '数据库表前缀';//'Database table prefix';
...
$lang['view_by_author']		= '按作者查看';//'View by author';
$lang['view_by_tag']		= '按标签查看';//'View by tag';
$lang['backup_version_prefix']	= '仅可导入相同版本emlog导出的数据库备份文件，且数据库表前缀需保持一致。';//'Only the database backup files exported by the same version of emlog can be imported, and the prefixes of the database tables must be consistent.';
$lang['db_prefix_current']	= '当前数据库表前缀';//'Current database table prefix';
$lang['cache_explain']		= '缓存可以加快站点的加载速度。通常系统会自动更新缓存，无需手动。有些特殊情况，比如缓存文件被修改、手动修改过数据库、页面出现异常等才需要手动更新。';//'Caching can speed up the loading speed of the site. Usually the system will automatically update the cache, no need to manually. Some special situations, such as cache files being modified, manual modification of the database, page exceptions, etc., need to be manually updated.';
