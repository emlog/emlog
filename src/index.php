...
if (!isset($action) || empty($action))
{
	...
	while($row=$DB->fetch_array($query1))
	{
		...
		//attachment
		!empty($log_cache_atts[$row['gid']]['attachment'])?
		$row['attachment'] = '<b>文件附件</b>:'.$log_cache_atts[$row['gid']]['attachment'] :
		$row['attachment'] = '';
		!empty($log_cache_atts[$row['gid']]['att_img'])?
		$row['att_img'] = $log_cache_atts[$row['gid']]['att_img']:
		$row['att_img'] = '';
		//tag
		$row['tag'] = !empty($log_cache_tags[$row['gid']])? '标签:'.$log_cache_tags[$row['gid']]:'';
		//e-mail
		$user_cache['mail']!=''?
		$row['name'] = "<a href=\"mailto:".$user_cache['mail']."\">".$user_cache['name']."</a>":
		$row['name'] = $user_cache['name'];
		//top
		$row['top']=='y'?
		$row['toplog'] = "<img src=\"./images/import.gif\" align=\"absmiddle\"  alt=\"推荐日志\" />":
		$row['toplog'] = '';
...
if ($action == 'showlog')
{
	...
	//tag
	!empty($log_cache_tags[$logid])?
	$tag = '标签:'.$log_cache_tags[$logid]:
	$tag = '';
	//attachment
	!empty($log_cache_atts[$logid]['attachment'])?
	$attachment = '<b>文件附件</b>:'.$log_cache_atts[$logid]['attachment']:
	$attachment = '';
	!empty($log_cache_atts[$logid]['att_img'])?
	$att_img = $log_cache_atts[$logid]['att_img']:
	$att_img = '';
	//comment	
	$comment_code=='y'?
	$cheackimg = "<img src=\"./lib/C_checkcode.php\" align=\"absmiddle\" /><input name=\"imgcode\"  type=\"text\" class=\"input\" size=\"5\">":
	$cheackimg = '';
	...
	while($s_com=$DB->fetch_array($query)){
		...
		$cmail!=''?
		$poster = "<a href=\"mailto:".$s_com['mail']."\">".$poster."</a>":
		$poster = $poster;
...
if($action == 'addcom')
{
	...
	elseif(strlen($comment)=='' or strlen($comment)>2000)
	...
	elseif($comment_code=='y' && $imgcode != $_SESSION['code'])
			...
			$sql="INSERT INTO ".$db_prefix."comment(date,gid,comment,mail,poster,hide)
					values('$localdate','$gid','$comment','$commail','$comname','$iscomment')";
			$DB->query($sql);
		if($iscomment == 'n'){
