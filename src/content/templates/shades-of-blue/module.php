...
function widget_blogger($title){
	...
	<b><?php echo $name; ?></b><br />
		...
		<div id='modbdes' style="display:none;">
...
function widget_twitter($title){
	...
	<li class="widget">
		...
		<div id='addtw' style="display: none;text-align:center">
...
function widget_music($title){
	...
	<li class="widget">
	<h2 class="widgettitle"><span onclick="showhidediv('blogmusic')"><?php echo $title; ?></span></h2>	
	<ul id="blogmusic">
...
function widget_search($title){ ?>
	<li class="widget">
	<h2 class="widgettitle"><span onclick="showhidediv('logserch')"><?php echo $title; ?></span></h2>
	<form name="keyform" id="searchform" method="get" action="./"><p>
	<input name="keyword"  type="text" value="" id="s" style="width:170px;"/>
	<input type="submit"  id="searchsubmit" value="搜索" onclick="return keyw()" />
	</form>
	</li>
<?php } ?>
<?php
...
function widget_bloginfo($title){
	...
	<li class="widget">
	<h2 class="widgettitle"><span onclick="showhidediv('bloginfo')"><?php echo $title; ?></span></h2>
	<ul id="bloginfo">
...
<?php 
//blog：分类和标签
function blog_sort_and_tag($sort, $blogid){
	global $log_cache_sort,$log_cache_tags; 
	if($log_cache_sort[$blogid] || !empty($log_cache_tags[$blogid])):
?>
	<div class="postmeta">
	<p>
	<?php if($log_cache_sort[$blogid]): ?>Filed under <a href="./?sort=<?php echo $sort; ?>"><?php echo $log_cache_sort[$blogid]; ?></a>
	<?php endif;?>
	<?php
...
function blog_trackback(){
	...
	$oddcomment = 'class="alt thread-alt" ';
?>
	<?php if($allow_tb == 'y'):?>
	<h2>引用地址：</h2>
	<center><input type="text" style="width:350px" id="trackback" value="<?php echo BLOG_URL;?>tb.php?sc=<?php echo $tbscode; ?>&amp;id=<?php echo $logid; ?>" /></center>
	<a name="tb"></a>
	<?php if($tb): ?>
	<h2>引用：</h2>
	...
	<li <?php echo $oddcomment; ?>>
		<b><a href="<?php echo $value['url'];?>" target="_blank"><?php echo $value['blog_name'];?></a></b>
		Says:
		<br />
	...
	<?php
		if($oddcomment == 'class="alt thread-alt" ')
			$oddcomment = 'class="even thread-even" ';
		else
			$oddcomment = 'class="alt thread-alt" ';
	?>
	<?php endforeach; ?>
...
function blog_comments(){
	...
	<li <?php echo $oddcomment; ?>>
		...
		<div id="replycomm<?php echo $value['cid']; ?>" class="reply"><?php echo $reply; ?></div>
		<?php if(ROLE == 'admin'): ?>
...
function blog_comments_post(){
	...
	<p><input type="hidden" name="gid" value="<?php echo $logid; ?>"  size="33" tabindex="1"/>
	<input type="text" name="comname" id="author" value="<?php echo $ckname; ?>" size="33" tabindex="1" />
	<label for="author"><small>昵称</small></label></p>
	<p><input type="text" name="commail" id="email" value="<?php echo $ckmail; ?>" size="33" tabindex="2" />
	<label for="email"><small>邮件地址（选填）</small></label></p>
	<p><input type="text" name="comurl" id="url" value="<?php echo $ckurl; ?>" size="33" tabindex="3" />
	<label for="url"><small>个人主页（选填）</small></label></p>
	<p><textarea name="comment" id="comment" cols="100%" rows="10" tabindex="4"></textarea></p>
	<p><?php echo preg_replace("/(.*?)class=\"input\"(.*?)/i","\\1id=\"imgcode\"\\2",$cheackimg);?><input name="submit" type="submit" id="submit" tabindex="5" value="发表评论" /></p>
	</form>
	</div>
	<?php endif; ?>
<?php }?>