...
function widget_blogger($title){
	...
	<ul class="sidemenu" style="text-align:center" id="bloggerinfo">
...
function widget_calendar($title){
	...
	<div id="calendar" class="sidemenu">
	</div>
...
function widget_twitter($title){
	...
	<?php if($index_twnum>0): ?>
		...
		<li id='addtw' style="display: none;text-align:center">
...
function widget_music($title){
	...
	<ul class="sidemenu" id="blogmusic">
...
function widget_newcomm($title){
	...
	<ul class="sidemenu" id="newcomment">
	<?php 
...
function widget_search($title){ ?>
	<h2><span onclick="showhidediv('logserch')"><?php echo $title; ?></span></h2>
	<form name="keyform" method="get" action="./" class="searchform">
	<p>
...
function widget_custom_text($title, $content, $id){ ?>
	...
	<ul class="sidemenu" id="<?php echo $id; ?>">
...
function blog_sort($sort, $blogid){
	...
	<span class="readmore">Posted in <a href="./?sort=<?php echo $sort; ?>"><?php echo $log_cache_sort[$blogid]; ?></a></span>
	<?php endif;?>
<?php }?>
<?php
//blog：文件附件
function blog_att($blogid){
	...
	$attachment = !empty($log_cache_atts[$blogid]) ? '<p><b>文件附件：</b>'.$log_cache_atts[$blogid] : '</p>';
	echo $attachment;
}
?>
<?php
...
function blog_trackback(){
	...
	<?php foreach($tb as $key=>$value):?>
		<li id="trackback">
		<p><a href="<?php echo $value['url'];?>" target="_blank"><?php echo $value['title'];?></a></p>
		<p>BLOG: <?php echo $value['blog_name'];?></p>
...
<?php if ($comments) : ?>
	...
	$reply = $value['reply']?"<b>博主回复：</b>{$value['reply']}":'';
	?>
...
function blog_comments_post(){
	...
	<label for="author"><small>昵称</small></label><input type="text" name="comname" maxlength="49" value="<?php echo $ckname; ?>"  size="22" tabindex="1">
	</p>
	<p>
	<label for="email"><small>邮件地址 (选填)</small></label><input type="text" name="commail"  maxlength="128"  value="<?php echo $ckmail; ?>" size="22" tabindex="2"> 
	</p>
	<p>
	<label for="url"><small>个人主页 (选填)</small></label><input type="text" name="comurl" maxlength="128"  value="<?php echo $ckurl; ?>" size="22" tabindex="3">
	</p>
	<p><textarea name="comment" id="comment" cols="100%" rows="10" tabindex="4"></textarea></p>
	<p><?php echo $cheackimg; ?></p>
	<p><input name="Submit" type="submit" id="comment_submit" value="发表评论" onclick="return checkform()" /></p>
	</form>
	<?php endif; ?>
<?php }?>