...
function widget_blogger($title){
	...
	<li><b><?php echo $name; ?></b></li>
		...
		<textarea name="bdes" class="input" id="bdes" style="overflow-y: hidden;width:150px;height:60px;"><?php echo $user_cache[1]['des']; ?></textarea>
		<br />
...
function blog_sort($sort, $blogid){
	...
	in <a href="./?sort=<?php echo $sort; ?>"><?php echo $log_cache_sort[$blogid]; ?></a>
	<?php endif;?>
<?php }?>
<?php
//blog：文件附件
...
function blog_trackback(){
	...
	<?php foreach($tb as $key=>$value):?>
		<li>
		<p class="comment-author">BLOG: <?php echo $value['blog_name'];?></p>
		<p><a href="<?php echo $value['url'];?>" target="_blank"><?php echo $value['title'];?></a></p>
		<p  class="comment-data"><?php echo $value['date'];?></p>
...
function blog_comments(){
	global $comments; ?>
<?php if ($comments) : ?>
	<h2 id="comments">评论：<a name="comment"></a></h2>
	<ol class="commentlist">
		<?php
			foreach($comments as $key=>$value):
			$reply = $value['reply']?"博主回复：{$value['reply']}":'';
		?>
			<li id="comment-<?php echo $value['cid']; ?>"><a name="<?php echo $value['cid']; ?>"></a>
				...
				<p class="comment-data"><div id="replycomm<?php echo $value['cid']; ?>"><?php echo $reply; ?></div></p>
		<?php if(ROLE == 'admin'): ?>
			...
			<?php if ($allow_remark == 'y') : ?>
				<h2 id="respond">发表评论</h2>
					<form action="./index.php?action=addcom" method="post" id="commentform">
						...
						<p><?php echo $cheackimg; ?></p><p><input name="submit" type="submit" id="submit" tabindex="5" value="发表评论" onclick="return checkform()" /></p>
					</form>
			<?php endif; ?>
<?php }?>