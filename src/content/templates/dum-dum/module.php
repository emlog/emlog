...
function widget_tag($title){
	...
	<div class="box3">
		...
		<?php foreach($tag_cache as $value): ?>
			<a style="font-size:<?php echo $value['fontsize']; ?>pt; height:30px;" href="./?tag=<?php echo $value['tagurl']; ?>" title="<?php echo $value['usenum']; ?> 篇日志"><?php echo $value['tagname']; ?></a>
		<?php endforeach; ?>			
		</ul>
		</div> 
	</div> 
...
function widget_search($title){ ?>
	<form name="keyform" method="get" action="./" id="searchform">
	<div>
...
function editflg($logid,$author){
	$editflg = ROLE == 'admin' || $author == UID ? '编辑:<a href="'.'./admin/write_log.php?action=edit&gid='.$logid.'">edit</a>' : '';
	echo $editflg;
}
?>
<?php 
...
function blog_sort($sort, $blogid){
	...
	分类:<a href="./?sort=<?php echo $sort; ?>"><?php echo $log_cache_sort[$blogid]; ?></a>
	<?php endif;?>
<?php }?>
<?php
//blog：文件附件
