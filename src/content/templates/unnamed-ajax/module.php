...
$isIE = strpos($ua,'MSIE');
$_ = (isset($_SERVER['HTTP_X_REQUESTED_WITH']) && $_SERVER['HTTP_X_REQUESTED_WITH'] == 'qiyuAjaxRequest') ? 0 : 1;
!$_ && header("HTTP/1.1 200 OK",true,200);
$site_title = $site_title . ' - powered by emlog';
?>
<?php
//widget：blogger
function widget_blogger($title){
	...
	<div id="blogger">
	<?php if (!empty($user_cache[1]['photo']['src'])): ?><img src="<?php echo BLOG_URL.$user_cache[1]['photo']['src']; ?>" width="<?php echo $user_cache[1]['photo']['width']; ?>" height="<?php echo $user_cache[1]['photo']['height']; ?>" alt="<?php echo $user_cache[1]['name']; ?>" /><?php endif;?>
...
function widget_calendar($title){ ?>
	...
	<script>unnamed.calendar('<?php echo Calendar::url(); ?>','calendar');</script>
	</div>
<?php }?>
<?php
//widget：标签
...
?>
	...
	<?php endforeach; ?>
	<?php if($hide_tag): ?><span id="show-tags" onclick="q('.hide-tags').show();q(this).remove();">更多&raquo;</span><?php endif; ?>
	</div>
	</div>
<?php }?>
<?php
...
function widget_newcomm($title,$_ = 1){
	...
	?>
	<li><img src="<?php echo getGravatar($value['mail'],32); ?>" class="side-avatar" width="32" height="32" alt="<?php echo $value['name']; ?>" title="<?php echo $value['name']; ?>" /><b><?php echo $value['name']; ?></b> 说：
	<br /><a href="<?php echo $url; ?>" title="<?php echo strip_tags($value['content']); ?>"><?php echo preg_replace("#\|ali(\d+)\|#i",'',$value['content']); ?></a></li>
	<?php endforeach; ?>
	<?php if($_): ?>
	</ul>
	</div>
...
function blog_trackback($tb, $tb_url, $allow_tb){
	...
	<div id="tb">
		...
		<?php if($tb): ?><div class="tb-count" onclick='q("#trackbacklist").toggle("normal")'>展开/折叠引用</div><?php endif; ?>
	</div>
	<?php endif; ?>
	<ul id="trackbacklist">
...
function blog_comments($comments){
	...
	<li class="comment<?php echo $alt;?>" id="comment-<?php echo $comment['cid']; ?>">
		...
		<?php if($isGravatar == 'y'): ?><div class="avatar"><img src="<?php echo getGravatar($comment['mail'],40); ?>" width="40" height="40" alt="<?php echo $comment['poster'];?>" title="<?php echo $comment['poster'];?>" /></div><?php endif; ?>
		<div class="comment-info">
			...
			<div class="comment-content"><?php echo translateSmile($comment['content']); ?></div>
		</div>
		<?php
			$alt = $alt == '' ? ' alt' : '';
...
<?php }?>
<?php
function translateSmile($comment) {
	//$comment = preg_replace('#(?:(?<smile>(?:\|ali\d+\|)+?)(?:\k<smile>)+)#i', '$1', $comment);
	$comment = preg_replace("#\|ali(\d+)\|#i", '<img src="'.BLOG_URL.'images/ali/$1.gif" id="ali$1" width="50" height="50" alt="阿狸$1" />', $comment);
	return $comment;
}
?>
<?php
		...
		<form method="post" name="commentform" action="<?php echo BLOG_URL; ?>index.php?action=addcom" id="commentform">
			...
			<div class="cmt-form-l">
				<div class="smiles" id="smiles">
					<?php for($i = 1; $i <= 20; $i++): ?>
					<img src="<?php echo BLOG_URL; ?>images/ali/<?php echo $i; ?>.gif" id="ali<?php echo $i; ?>" width="24" height="24" alt="阿狸<?php echo $i; ?>" />
					<?php endfor; ?>
				</div>
...
?>
	...
	<?php 
		...
		$current_tab = (BLOG_URL . $path == $value['url']) ? 'current ' : '';
		?>
		<li class="<?php echo $current_tab;?>navlist"><a href="<?php echo $value['url']; ?>"<?php echo $newtab;?>><?php echo $value['naviname']; ?></a></li>
	<?php endforeach;
}
