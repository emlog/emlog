...
/*vot*/        $val['Name'] = "<a href=\"./plugin.php?plugin={$val['Plugin']}\" title=\"".lang('plugin_settings_click')."\">{$val['Name']} <img src=\"./views/images/set.png\" border=\"0\"></a>";
    ...
    ?>	
      <tr>
        ...
        <a href="./plugin.php?action=<?= $plug_action ?>&plugin=<?= $key ?>&token=<?= LoginAuth::genToken() ?>"><img src="./views/<?php echo ADMIN_TEMPLATE; ?>/images/plugin_<?= $plug_state ?>.gif" title="<?= $plug_state_des ?>" align="absmiddle" border="0"></a>
        </td>
