...
function em_confirm(id, property, token) {
    ...
    switch (property) {
        ...
        case 'reset_widget':
            ...
            msg = '你确定要恢复组件设置到初始状态吗？这样会丢失你自定义的组件。';
            break;
        case 'plu':
...
function autosave(act) {
        ...
        if (act == 3 || act == 4) {
            ...
            var url = "page.php?action=autosave",
                ...
                querystr = "content=" + encodeURIComponent(content) + "&title=" + encodeURIComponent(title) + "&alias=" + encodeURIComponent(alias) + "&template=" + encodeURIComponent(template) + "&allow_remark=" + allow_remark + "&is_blank=" + is_blank + "&url=" + pageurl + "&token=" + token + "&ishide=" + ishide + "&as_logid=" + logid;
        } else {
            editorMap['content'].sync();
